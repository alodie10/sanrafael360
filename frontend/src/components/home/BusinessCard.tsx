...
export default function BusinessCard({ negocio, index = 0 }: BusinessCardProps) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      whileHover={{ y: -8 }}
      className="group relative bg-slate-900/40 rounded-[2.5rem] overflow-hidden border border-white/5 hover:border-primary/40 transition-all duration-500 shadow-2xl backdrop-blur-sm"
    >
      {/* Portada con Zoom */}
