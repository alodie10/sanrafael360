...
<div style="font-family: sans-serif; padding: 30px; border: 1px solid #e2e8f0; border-radius: 16px; max-width: 600px; margin: 0 auto; color: #1e293b;">
  ...
  <div style="background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #f1f5f9; margin: 20px 0;">
    <p style="margin: 5px 0;"><strong>Negocio:</strong> ${escapeHtml(negocioName)}</p>
    <p style="margin: 5px 0;"><strong>Solicitante:</strong> ${escapeHtml(userEmail)}</p>
    <p style="margin: 5px 0;"><strong>Mensaje:</strong> ${message ? escapeHtml(message) : '<em>Sin mensaje adjunto</em>'}</p>
  </div>
...
<div style="font-family: sans-serif; padding: 25px; border: 1px solid #eee; border-radius: 12px; max-width: 600px;">
  ...
  <p>Hola, el equipo de <b>San Rafael 360</b> ha procesado tu solicitud para <b>${escapeHtml(negocioName)}</b>.</p>
