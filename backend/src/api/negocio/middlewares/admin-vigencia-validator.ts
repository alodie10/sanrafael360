...
export default (_config: unknown, _ctx: { strapi: any }) => {
  return async (ctx: any, next: () => Promise<void>) => {
    ...
    if (prospector_valid_until != null && Number.isNaN(Date.parse(String(prospector_valid_until)))) {
      throw new ValidationError('prospector_valid_until debe ser una fecha válida');
    }
    await next();
  };
};
