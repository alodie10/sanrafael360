 ...
 */
export function wrapClienteAvisosEmail(
  bodyHtml: string,
  options: { isTest?: boolean; unsubscribeUrl?: string } = {}
): string {
  ...
  const baja = options.unsubscribeUrl
    ? `Si preferís no recibir estos mensajes, <a href="${options.unsubscribeUrl}" style="color:#0f172a;text-decoration:underline;">darte de baja con un clic</a>.`
    : `Si preferís no recibir estos mensajes, escribinos a <a href="mailto:hola@sanrafael360.com" style="color:#0f172a;">hola@sanrafael360.com</a>.`;
