...
export function slotsFromMensaje(mensaje: string): PlantillaSlot[] {
  const texto = String(mensaje || '').trim();
  return DEFAULT_SLOT_TITULOS.map((titulo, i) => ({
    titulo,
    texto: i === 0 ? texto : '',
  }));
}
...
export function pickSlotTexto(slots: PlantillaSlot[], index: unknown): { index: number; slot: PlantillaSlot } {
  const i = parsePlantillaIndex(index);
